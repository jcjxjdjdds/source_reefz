...
zel_dev = (6556390631, 6909581339, 5675627801)
...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 VeGa - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/VeGaSouRce) 🗑♻️𓆪\n**𓍹━─━─━─━─VeGa─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id == 6556390631 or user.id == 5675627801 or user.id == 6909581339:
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين المساعديـن  ❏╰**")
    if user.id == 6556390631 or user.id == 6556390631 or user.id == 6556390631:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        mention,
...
async def permalink(mention):
    ...
    if user.id == 6556390631 or user.id == 6556390631 or user.id == 6556390631:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا محمد الهيبـه . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
